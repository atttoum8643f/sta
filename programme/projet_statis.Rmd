...
Pour quelques visualisations nous allons utiliser un tableau à trois entrées représentant la pollution en occitanie de 2021 à 2023. Les individus $i$ sont des stations de mesures (on en a 9), les variables $j$ sont les polluants (NO=monoxyde d'azote, NO2=dioxyde d'azote, O2=oxygène et PM10=particules inférieures) et les tableaux $k$ sont les années (2021, 2022, 2023).

```{r}
# Tableau à trois dimension
donnees_2021 <- read.csv("2021_filtre.csv", stringsAsFactors = FALSE, row.names=1)
donnees_2021 <- as.matrix(apply(donnees_2021, 2, as.numeric))
donnees_2022 <- read.csv("2022_filtre.csv", stringsAsFactors = FALSE, row.names=1)
donnees_2022 <- as.matrix(apply(donnees_2022, 2, as.numeric))
donnees_2023 <- read.csv("2023_filtre.csv", stringsAsFactors = FALSE, row.names=1)
donnees_2023 <- as.matrix(apply(donnees_2023, 2, as.numeric))

n_tableaux = list(donnees_2021,donnees_2022,donnees_2023)

resultats_n = matcoef_RV(n_tableaux)
```
...
On a par suite: \( u'(*)\) et \( (**) \Rightarrow u'Ru = \lambda \).
...
La fonction calculant les composantes, la représentation des individus et celle des tableaux setrouvent en annexe. Nous verrons ci-dessous un exemple d'application de ces fonctions sur les données simulated.
```{r,include=FALSE}
cmp <- function(T_tabl,k){
  ...
  cmp_k <- Y_nrm %*% u[,k]
  F_k <- matrix(cmp_k, nrow = n, ncol = p)
  rownames(F_k) <- rownames(a)
  return(list(u = u, lambda = lambda, F_k = F_k))
}

comp_prin = cmp(n_tableaux,1)$F_k
```


```{r,fig.height=4.8,fig.width=5.5,fig.align='center',include=FALSE}
representation_graph_ind <- function(F_t, k, l, 
                                     aff_noms = FALSE, col = "navy", cex_titre = 1) {
  if (l <= k) {
    print(paste("Le 1er axe doit être strictement plus petit que le 2nd "))
    return(NULL)
  }
  ...
  if (aff_noms == TRUE) {
    text(df$PC_k, df$PC_l, labels = rownames(df), cex = 0.85, col = col, pos = ifelse(df$PC_k >= 0, 4, 2))
  }
...
# Afficher les graphiques pour les composantes principales de 1 à 10
for (i in 1:4) {
  Ft <- cmp(n_tableaux,i)$F_k
  representation_graph_ind(Ft, 1, 2, aff_noms = TRUE,cex_titre = 0.65)
}
```


```{r,include=FALSE}
geo_ind = function(F_t, k, l, col_points = "black", 
  ...
  plot <- ggplot(df, aes(x = PC_k, y = PC_l)) +
...
cosinus(A,cmp(n_tableaux,1)$F_k) 
```


```{r,fig.height=3.9,fig.width=4.8,fig.align='center',include=FALSE}
representation_graph_tab = function(T_tab, k, l){
  if (l <= k) {
  ...
  row_names <- if (is.null(names(T_tab))) paste("X", 1:t) else names(T_tab)
...
representation_graph_tab(n_tableaux, 1, 2)
...
\[\Gamma = \begin{pmatrix}
1.0000 & 0.5698 & -0.3692 \\
0.5698 & 1.0000 & 0.0771 \\
-0.3692 & 0.0771 & 1.0000 \\
\end{pmatrix}\]
...
# Situation 2
Dans cette partie allons utilier les data "chickenk" du package "ade4" qui contient les données de 351 poulets sur 5 différents groupes de vairables. Les 5 tableaux décrivent : \
- la mortalité en 4 variables, \
- la strucuture de la ferme dans laquelle ils on été élevé en 5 variables, \
- les charactéristiques de son troupeau au placement du poulet en 4 variables, \
- les charactéristiques du troupeau pendant l'élevage en 6 variables, \
- et les conditions de transport, d'entreposage, d'abattoir et d'inspection en 5 variables). 
...
```{r}
# Définir la disposition en grille des graphiques
par(mfrow = c(1, 2))

# Afficher les graphiques pour les composantes principales de 1 à 10
for (i in 1:4) {
  Ft <- cmp(lst, i)$F_k
  representation_graph_ind(Ft, 1, 2, aff_noms = TRUE,cex_titre = 0.75)
}

representation_graph_tab(lst, 1, 2)
```
...
representation_graph_ind <- function(F_t, k, l, 
  ...
  # Plot les composantes principales dans un plan avec l'origine au centre
  plot(df$PC_k, df$PC_l, xlab = paste("PC", k), ylab = paste("PC", l),
       main = paste("Représentation des individus dans le plan (", k, ",", l, ")"),
       xlim = limits, ylim = limits, pch = 22, cex = 0.75, col = "black", 
       bg = "#222211")
