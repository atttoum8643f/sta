...
a) Ecrivons le produit scalaire sous forme \(tr(\tilde{A}'\tilde{B})\) (produit scalaire de Frobénius) en explicitant la transformation \(Z \quad vers \quad \tilde{Z}\), \( \forall Z \) \( (n, p) \).
...
\begin{align*}
[A \mid A] &= 0\\
\iff \text{tr}(C^{\frac{1}{2}} A' W^{\frac{1}{2}} W^{\frac{1}{2}} A C^{\frac{1}{2}}) &=0\\
\iff tr(\tilde{A}'\tilde{A}) &=0\\
\iff sum_{l=1}^{p} \sum_{k=1}^{n} \tilde{a}_{kl} \tilde{a}_{kl} &=0\\
\iff sum_{l=1}^{p} \sum_{k=1}^{n} \tilde{a}_{kl}^{2} &=0\\
\iff \tilde{a}_{kl} &=0 \quad \forall k,l \in \{1,2,...,n\} \text{ et } \{1,2,...,p\
\iff \tilde{A} &=0 \\
\iff A &=0 
\end{align*} 
(car \(W\) et \(C\) sont régulières)
...
c) Écrivons le produit scalaire précédent sous forme de double somme. On reprend [A \mid B]=\text{tr}(C^{\frac{1}{2}} A' W^{\frac{1}{2}} W^{\frac{1}{2}} B C^{\frac{1}{2}}) = tr(\tilde{A}'\tilde{B}). On note \(\tilde{A} = (\tilde{a}_{ij})\) et \(\tilde{B} = (\tilde{b}_{ij})\) avec \(\tilde{A} = W^{\frac{1}{2}}AC^{\frac{1}{2}})\) et \(\tilde{B} = W^{\frac{1}{2}}BC^{\frac{1}{2}})\), d'où \(tr(\tilde{A} \tilde{B}) = \sum_{k=1}^{n} \tilde{a}_{ik} \tilde{b}_{kj}\). Ainsi:
\begin{align*}
...
D'après, $(S)$ on en déduit que les vecteurs u solution du premier ordre sont les vecteurs propres de la matrice \(\Gamma = \sum_{t=1}^{T}\sum_{\tau=1}^{T}\frac{\tilde{X_t'}\tilde{X_{\tau}} }{\left[\left| X_t \right|\right]\left[\left| X_{\tau} \right|\right]} \) des coefficients RV d'Escoufier entre T tableaux \(X_t(n , p)\).
...
comp_prin = composantes_principales(vec_prop, mat_jux)
print(comp_prin[,1]) # Affichage de la première composante principale
```
...
representation_graphique(vec_prop, mat_jux, 1, 3) # Affichage de la représentation graphique des individus en plan principal (1,2)
