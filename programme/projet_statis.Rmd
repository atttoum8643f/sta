...
```
On veut prouver mathématiquement que les vecteurs u forment une base I-orthonormée. Pour cela, nous devons montrer que les vecteurs u sont orthogonaux entre eux et que leur norme est égale à 1.
...
b) Les matrices \(P_m\) se trouvent dans l'espace \(\mathbk{P} = \mathbk{M}_n(\mathbb{R})\) où \(n\) est le nombre d'individus.

1.2.a) Les poids naturel des lignes de \(P_m\) serait \(W = frac{1}{n}I_n \) même chose ^pour les colonnes.

b) On peut munir \(\mathbk{P}\) du produit scalaire suivant:
...
mat_rv = data.frame(statis::coef_RV(data))
mat_rv
```
